...
def run(request):    
    ...
    try:
        ...
        logging.info('k_anonymity file unknown error: ' + inputFile)
    
    file.log = "Get file success!!"
    logging.info('k_anonymity Get file success!!')
    ...
    logging.info('k_anonymity Get SA success!!')
    ...
    try:
        ...
        logging.critical('k_anonymity unknown error, stop run', exc_info=True)
    
    
    file.log = "Get Dict File success!!"
    logging.info('k_anonymity Get Dict File success!!')
    ...
    def find_interval(x,att):
        ...
        for i in dic[att]['interval']:
            if x >= i[0] and x <= i[1]:
                return i
        logging.critical('k_anonymity can\'t find_interval!!')
    
    logging.info('k_anonymity total record = ' + str(len(df)))    
    
    # 輸入 K
    try:
        k = int(request.GET.get('k', None))
        if(k > len(df)):
            logging.critical('k_anonymity k > len(df)')
            raise ParameterException(gettext('k 數值不能大於資料筆數'))
        elif(k <= 0):
            logging.critical('k_anonymity k <= 0')
            raise ParameterException(gettext('k 數值不能小於0'))
    except:
        logging.critical('k_anonymity k is not number')
        raise ParameterException(gettext('k 並非數字'))
    
    logging.info('k_anonymity Get K success!!')
    ...
    logging.info('k_anonymity total diversity = ' + str(total_diversity))
...
# 建立 Distance Matrix (numpy)
    ...
    for att in cate_data:
        logging.debug('k_anonymity df[att].value_counts(): ' + str(df[att].value_counts()))
        logging.debug('k_anonymity df[att].value_counts().index[0]: ' + str(df[att].value_counts().index[0]))
...
# 使用 center，建立 Distance Center List (list)
    ...
    for x in range(len(df)):
        ...
        if glist[r] == 1:
            ...
            group = []
            group.append(r)
            logging.info('k_anonymity start first stage(add diversity), grouping = '+group)
            ...
            while distance_list[r2]!=np.inf :
                ...
                r2 = np.where(distance_list==np.min(distance_list))[0][0]
            logging.info('k_anonymity first stage finish, grouping = '+group)
            file.log = str(upgroup_num)  + "/" + str(len(df)) + " 完成第一階段"
            file.save()
            
            # extend the group
            logging.info('k_anonymity start second stage(extend the group), grouping = '+group)
                ...
                for i in range(len(df)):
                    ...
                    while d_in_list[r3]!=np.inf:
                        ...
                        if after_diversity>=original_diversity and d_out > d_in_list[r3]:
                            ...
                            d_in_list[r3] = np.inf
                            r3 = np.where(d_in_list==np.min(d_in_list))[0][0]
            logging.info('k_anonymity second stage finish, grouping = '+group)
            file.log = str(upgroup_num)  + "/" + str(len(df)) + " 完成第二階段"
            file.save()
            
            logging.info('k_anonymity start third stage(extend the size to k), grouping = '+group)
            ...
            if len(group) < k:                
                ...
                for i in range(k-len(group)):
                    ...
                    while glist[r5]==0:
                        logging.critical('k_anonymity unknown error, glist[r5]==0')
            ...
            logging.info('k_anonymity third stage finish, grouping = '+group)
            file.log = str(upgroup_num)  + "/" + str(len(df)) + " 完成第三階段"
            file.save()
            
            # 將 group 放入 final_group清單中
            final_group.append(group)
            logging.info('k_anonymity push group to final_group')
            upgroup_num = upgroup_num + 1
            
    # 將剩下 ungroup 的點加入鄰近的 group
    logging.info('k_anonymity final stage(add ungroup to grouping)')
        ...
        if glist[i] == 1:
            ...
            for group in final_group:
                if r4 in group:
                    group.append(i)
                    glist[i] = 0
    logging.info('k_anonymity finish final stage')
    
    a = 0
    for g in final_group:
        a = a + len(g)
        
    logging.debug('k_anonymity total record after grouping = ' + str(a))
...
# 檢查每個 group 是否符合 k l
    logging.info('k_anonymity check group')
    
    for g in final_group:
        if len(g) < k:
            logging.info('k_anonymity group = ' + g + ', len(g) < k')
        
        diversity = {}
        for i in g:
            diversity[df.iloc[i][sensitiveAttribute]] = 1
        if len(diversity) < l:
            logging.info('k_anonymity group = ' + g + ', len(diversity) < l')
...
# 設定顯示全部資料
    ...
    logging.info('k_anonymity All Success!!')
