...
def prepare_creditcard(data_path: str):
...
def train_split(site_num, X, y):
    # Calculate the number of samples per fold
    samples_per_fold = len(X) // site_num

    # Create empty lists to store all the X_train and X_val datasets
    all_X_train = []
    all_X_val = []
    all_y_train = []
    all_y_val = []
    ...
    for fold in range(site_num):
        ...
        # Append the current X_train and X_val to the lists
    ...
    # Concatenate all the X_train and X_val datasets from each site
    train_data = pd.concat(all_X_train)
    train_label = pd.concat(all_y_train)
    val_data = pd.concat(all_X_val)
    val_label = pd.concat(all_y_val)
    print("Class Proportions:")
    print(val_label.value_counts(normalize=True))
...
def main():
    ...
    # Load data
    start = time.time()
    X, y = prepare_creditcard(data_path)
    X_train, y_train, X_val, y_val = train_split(site_num, X, y)
