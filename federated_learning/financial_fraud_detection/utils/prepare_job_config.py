...
def job_config_args_parser():
    ...
    parser.add_argument("--round_num", type=int, default=101, help="Total number of training rounds")
    parser.add_argument(
    ...
    parser.add_argument(
        "--DP", type=str, default="None", help="privacy protecrion mode: Differential privacy"
    )
    return parser
...
def _get_src_job_dir(training_mode, DP):
    if DP == "None":
        strDP = ""
    elif DP == "DP":
        strDP = "_DP"
    ...
    base_job = base_job_map[training_mode] + strDP
    return pathlib.Path(MODE_ALGO_MAP[training_mode]) / JOB_CONFIGS_ROOT / base_job
...
def main():
    ...
    job_name = _get_job_name(args)   #"creditcard_" DP str(args.site_num)"_"args.training_mode"_"args.split_method"_split_"args.lr_mode"_lr"
    src_job_path = _get_src_job_dir(args.training_mode, args.DP)
    print("src_job_path:", src_job_path)
